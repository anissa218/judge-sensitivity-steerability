#!/usr/bin/env python3
"""Run sensitivity & steerability experiments for one or more judges.
...
def main() -> None:
    ...
    p.add_argument("--experiments", default="sensitivity,steerability",
                   help="comma list: sensitivity, steerability, or fine-grained names")
    ...
    eval_df = datasets.load(args.dataset, mode, data_path=args.data, policy=pol)
    family = eval_df.attrs["family"]
    if args.max_samples:
        eval_df = eval_df.head(args.max_samples).reset_index(drop=True)
        eval_df.attrs["family"] = family
    print(f"  family={family}  eval_rows={len(eval_df)}")

    conds = experiments.build_conditions(eval_df, family, mode, exps)
