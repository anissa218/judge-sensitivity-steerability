# judge-sensitivity-steerability
...
- **Sensitivity**: does a judge *use* in-context information? We test the impact
  of, when a dataset provides it, **novel context** (i.e. additional task
  information, correct and irrelevant).
...
python run_experiments.py \
  ...
  --experiments sensitivity,steerability --seeds 5
```
...
|---|---|
| `sens_context` (only if the dataset has a context column) | `context_none` (base), `context_correct`, `context_shuffled` |
...
full sensitivity + steerability suite works on your categories:
...
src/jss/
  ...
  templates.py         # prompt template selection + rendering
